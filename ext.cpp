...
namespace recviewer {
  namespace ext {
    ...
    static af::shared<vec2<double> > get_target_pixels(dxtbx::model::Panel panel, vec3<double> s0, int xlim, int ylim, double maxres)
    {
    ...
    static void fill_voxels(const af::flex_int &image, af::flex_double &grid, 
      			    af::flex_int &cnts, const flex_vec3_double &rotated_S,
			    const flex_vec2_double &xy, const double rec_range) {
	...
	int x = xy[i][0];
        ...
        if (ind_x >= npoints || ind_y >= npoints || ind_z >= npoints || ind_x < 0 || ind_y < 0 || ind_z < 0) continue;
